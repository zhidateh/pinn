...
class PINN:
    ...
    def __init__(self, P_back, x, y, P, rho, u, v, E, layers):
        ...
        self.P_pred, self.rho_pred, self.u_pred,self.v_pred, self.E_pred, self.e_res = self.net_NS(self.P_back_tf, self.x_tf,self.y_tf)
        ...
        self.loss = tf.reduce_mean(tf.square(self.P_tf - self.P_pred))/(P_norm**2) + \
                    ...
                    tf.reduce_mean(tf.square(self.E_tf - self.E_pred))/(E_norm**2) + \
    ...
    def net_NS(self, P_back, x, y):
        P_rho_u_v_E = self.neural_net(tf.concat([P_back, x, y], 1), self.weights, self.biases)
        P   = P_rho_u_v_E[:,0:1]
        rho = P_rho_u_v_E[:,1:2]
        u   = P_rho_u_v_E[:,2:3]
        v   = P_rho_u_v_E[:,3:4]
    ...
    def train(self, nIter): 
        ...
        tf_dict = {self.P_back_tf: self.P_back, self.x_tf: self.x, self.y_tf: self.y,
                    self.P_tf: self.P, self.rho_tf: self.rho, self.u_tf: self.u, self.v_tf: self.v,self.E_tf: self.E
                    }
